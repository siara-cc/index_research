...
class sqlite : public bplus_tree_handler<sqlite>, public sqlite_common {
    ...
    public:
        ...
        void make_space() {
            ...
            const int data_size = block_sz - get_kv_last_pos();
            ...
            for (new_idx = 0; new_idx < orig_filled_size; new_idx++) {
                ...
                set_ptr(new_idx, block_sz - new_data_len);
            }
            int new_kv_last_pos = block_sz - new_data_len;
        ...
        uint8_t *split(uint8_t *first_key, int *first_len_ptr) {
            int orig_filled_size = filled_size();
            uint32_t SQLT_NODE_SIZE = is_leaf() ? block_size : parent_block_size;
            int lvl = current_block[0] & 0x1F;
