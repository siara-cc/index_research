...
byte *dft_node_handler::split(byte *first_key, int16_t *first_len_ptr) {
    ...
    int16_t idx = 0;
    keyPos = 0;
    byte tp[DFT_MAX_KEY_PREFIX_LEN];
    byte brk_tp_old[DFT_MAX_KEY_PREFIX_LEN];
    byte brk_tp_new[DFT_MAX_KEY_PREFIX_LEN];
    byte *t_end = trie + DFT_TRIE_LEN;
    ...
    idx = *first_len_ptr;
    int16_t k = brk_trie_pos - DFT_UNIT_SIZE;
    while (idx--) {
        int j = brk_tp_new[idx] - 1;
        first_key[idx] = trie[j];
        if (*first_len_ptr > (idx + 1)) {
    ...
    new_block.key_at = new_block.buf + brk_kv_pos + 1;
    new_block.key_at_len = new_block.buf[brk_kv_pos];
    BPT_TRIE_LEN= brk_trie_pos;
    idx = brk_tp_old_pos;
    while (idx--) {
        int j = brk_tp_old[idx];
#if DFT_UNIT_SIZE == 3
        trie[j] &= xC0;
#else
        trie[j] &= x80;
...
#if DFT_UNIT_SIZE == 3
    if ((DFT_TRIE_LEN+ need_count) > 189) {
        ...
        //}
            ...
            if ((getKVLastPos() - kv_len - 2)
                    < (DFT_HDR_SIZE + DFT_TRIE_LEN + need_count)) {
                return true;
            }
            if (DFT_TRIE_LEN + need_count > 240) {
                return true;
            }
            return false;
        }
...
void dft_node_handler::traverseToLeaf(byte *node_paths[]) {
    byte level;
    level = 1;
    if (isPut)
        *node_paths = buf;
    while (!isLeaf()) {
        ...
        if (idx < 0)
            idx = -idx;
#if DFT_UNIT_SIZE == 3
        key_at = buf + get9bitPtr(trie + idx + 1);
#else
        key_at = buf + util::getInt(trie + idx + 2);
#endif
        ...
        while (key_char > *t) {
            ...
            if (s == 0) {
                ...
                if (isPut) {
                    ...
                    need_count = DFT_UNIT_SIZE;
                    triePos = t;
                }
                return trie - t + DFT_UNIT_SIZE;
            }
            s *= DFT_UNIT_SIZE;
            t += s;
            t--;
            ...
            case 2:
                ...
                if (cmp < 0) {
                    ...
                    while (t > trie) {
                        t -= DFT_UNIT_SIZE;
#if DFT_UNIT_SIZE == 3
                        int16_t ptr = get9bitPtr(t + 1);
#else
                        int16_t ptr = util::getInt(t + 2);
#endif
                        if (ptr)
                            break;
                    }
...
void dft_node_handler::appendPtr(int16_t p) {
    ...
    trie[DFT_TRIE_LEN] = p;
    if (p & x100)
    trie[DFT_TRIE_LEN - 1] |= x80;
    else
    trie[DFT_TRIE_LEN - 1] &= x7F;
    DFT_TRIE_LEN++;
#else
    util::setInt(trie + DFT_TRIE_LEN, p);
    DFT_TRIE_LEN += 2;
#endif
}
