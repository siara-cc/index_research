...
int16_t dfox_node_handler::nextKey(dfox_iterator_status& s) {
    if (s.t == trie) {
        ...
        while (s.offset_a[keyPos] == 8 && (s.tc_a[keyPos] & x04))
            s.offset_a[--keyPos]++;
    }
    do {
        if (s.offset_a[keyPos] > x07) {
            s.tp[keyPos] = s.t;
            s.tc_a[keyPos] = *s.t++;
            s.child_a[keyPos] = s.leaf_a[keyPos] = 0;
            if (s.tc_a[keyPos] & x02)
                s.child_a[keyPos] = *s.t++;
            if (s.tc_a[keyPos] & x01)
                s.leaf_a[keyPos] = *s.t++;
            //s.child_a[keyPos] = (s.tc_a[keyPos] & x02 ? *s.t++ : x00);
            //s.leaf_a[keyPos] = (s.tc_a[keyPos] & x01 ? *s.t++ : x00);
            s.offset_a[keyPos] = x00;
        }
...
byte *dfox_node_handler::split(int16_t *pbrk_idx, byte *first_key,
    ...
    deleteTrieLastHalf(brk_key, brk_key_len, s);
    ...
    deleteTrieFirstHalf(first_key, new_brk_key_len, s, new_block);
...
void dfox_node_handler::deleteTrieLastHalf(byte *brk_key, int16_t brk_key_len, dfox_iterator_status& s) {
    register byte *t = 0;
    register byte children = 0;
    for (register int idx = 0; idx < brk_key_len; idx++) {
        int offset = s.offset_a[idx];
        if (idx == brk_key_len - 1)
            offset--;
        byte tc, leaves;
        t = s.tp[idx];
        register byte *prev_t = t;
        tc = *t++;
        children = (tc & x02 ? *t++ : x00);
        children &= (
                (idx == brk_key_len - 1) ?
                        left_mask[offset] : left_incl_mask[offset]);
        leaves = (tc & x01 ? left_incl_mask[offset] & *t++ : x00);
        *prev_t++ = (tc | x04);
        if (tc & x02)
            *prev_t++ = children;
        if (tc & x01)
...
void dfox_node_handler::deleteTrieFirstHalf(byte *brk_key,
        int16_t brk_key_len, dfox_iterator_status& s, dfox_node_handler& new_block) {
    byte children;
    register byte *delete_start = new_block.trie;
    register int tot_del = 0;
    for (register int idx = 0; idx < brk_key_len; idx++) {
        register int offset = s.offset_a[idx];
        if (idx == brk_key_len - 1)
            offset--;
        byte tc, leaves;
        byte *t = new_block.trie + (s.tp[idx] - trie) - tot_del;
        register byte *prev_t = t;
        if (1) { //idx) {
            int to_delete = prev_t - delete_start;
            if (to_delete) {
                new_block.TRIE_LEN -= to_delete;
                t -= to_delete;
                memmove(delete_start, prev_t,
                        new_block.trie + new_block.TRIE_LEN - delete_start);
                prev_t -= to_delete;
                tot_del += to_delete;
            }
        }
        ...
        children = (tc & x02 ? *t++ : x00);
        leaves = (tc & x01 ? *t++ : x00);
        int to_skip = GenTree::bit_count[children & left_mask[offset]];
        leaves &= (
                (idx == brk_key_len - 1) ?
...
void dfox_node_handler::insertCurrent() {
    ...
    case INSERT_MIDDLE1:
        *origPos &= xFB;
        insAt(triePos - trie, ((key_char & xF8) | x05), mask);
        break;
    case INSERT_MIDDLE2:
        insAt(triePos - trie, ((key_char & xF8) | x01), mask);
        break;
    case INSERT_LEAF:
        origTC = *origPos;
    ...
    case INSERT_THREAD:
        ...
        while (p < min) {
            ...
            switch (c1 == c2 ?
                    (p + 1 == min) ? 0 : 1 : ((c1 ^ c2) < x08 ? 2 : 3)) {
            case 3:
                triePos += ins4BytesAt(triePos - trie, (c1 & xF8) | x01,
                        x80 >> (c1 & x07), (c2 & xF8) | x05, x80 >> (c2 & x07));
                break;
            case 2:
                triePos += insAt(triePos - trie, (c1 & xF8) | x05,
                        (x80 >> (c1 & x07)) | (x80 >> (c2 & x07)));
                break;
            case 1:
                triePos += insAt(triePos - trie, (c1 & xF8) | x06,
                        x80 >> (c1 & x07));
                break;
            case 0:
                triePos += insChildAndLeafAt(triePos - trie, (c1 & xF8) | x07,
                        x80 >> (c1 & x07));
                break;
            }
            if (c1 != c2)
        ...
        if (c1 == c2) {
            ...
            insAt(triePos - trie, (c2 & xF8) | x05, (x80 >> (c2 & x07)));
        }
        min = p - keyPos;
        keyPos = p + 1;
        if (min < key_at_len)
...
int16_t dfox_node_handler::locate(int16_t level) {
    ...
    do {
        ...
        case 1:
            ...
            break;
        case 0:
            register int r_leaves, r_children, r_mask;
            origPos = t++;
...
byte dfox_node_handler::ins4BytesAt(byte pos, byte b1, byte b2, byte b3,
        byte b4) {
    register byte *ptr = trie + pos;
