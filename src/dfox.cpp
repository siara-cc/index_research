...
void dfox_node_handler::insertCurrent() {
    ...
    case INSERT_CONVERT:
        ...
        if (cmp_rel == 0) {
            ...
            while (to_skip) {
                ...
                if (child_tc & x02)
                    to_skip += util::bit_count[*triePos++];
                triePos++;
                if (child_tc & x04)
                    to_skip--;
            }
        ...
        *triePos++ = ((cmp_rel == 0 ? c : key_char) & xF8) | b;
        b = (cmp_rel == 1 ? (*origPos == x03 ? 3 : 4) : (*origPos == x03 ? 1 : 2));
        if (diff) {
            b++;
        ...
        *triePos++ = 1 << ((cmp_rel == 0 ? c : key_char) & x07);
        ...
        if (cmp_rel == 1) {
            ...
            *triePos++ = 1 << ((cmp_rel == 0 ? key_char : c) & x07);
        need_count--;
        need_count -= diff;
        if (need_count)
    ...
    case INSERT_THREAD:
        ...
        if (p < min) {
            if (need_count) {
                insAt(origPos + 3, (need_count << 1) | x01, key + keyPos, need_count);
                triePos += need_count;
                triePos++;
                p += need_count;
            }
