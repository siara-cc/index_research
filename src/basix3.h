...
public:
    ...
    basix3(uint32_t leaf_block_sz = DEFAULT_LEAF_BLOCK_SIZE,
        ...
        bplus_tree_handler<basix3>(leaf_block_sz, parent_block_sz, cache_sz, fname, 0, false, opts) {
    }
    ...
    basix3(const char *filename, int blk_size, int page_resv_bytes, const uint8_t opts) :
       bplus_tree_handler<basix3>(filename, blk_size, page_resv_bytes, opts) {
        init_stats();
    }
