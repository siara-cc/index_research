...
int16_t bft_node_handler::insertCurrent() {
    ...
    case INSERT_THREAD:
        ...
        childPos = origPos + 1;
        triePos = childPos + 1;
        *childPos |= ((TRIE_LEN - (triePos - trie - 2)) / 3);
        p = keyPos;
    ...
    do {
        ...
        case 0:
            ...
            if (*t & 0x40) {
                ...
                if (r_children)
                    keyFoundAt = buf + getLastPtrOfChild(t + r_children * 3);
                else
        ...
        //} else if (key_char == trie_char) {
            ...
            ptr = get9bitPtr(t);
            switch (ptr ?
                    (r_children & 0x3F ? (keyPos == key_len ? 3 : 1) : 2) :
                    (r_children & 0x3F ? (keyPos == key_len ? 0 : 4) : 0)) {
            case 2:
                int16_t cmp;
                key_at = (char *) buf + ptr;
                key_at_len = *key_at;
...
int16_t bft_node_handler::locate() {
    ...
    do {
        ...
        case 1:
            ...
            switch (ptr ?
                    (r_children & 0x3F ? (keyPos == key_len ? 3 : 1) : 2) :
                    (r_children & 0x3F ? (keyPos == key_len ? 0 : 1) : 0)) {
            case 2:
                int16_t cmp;
                key_at = (char *) buf + ptr;
                key_at_len = *key_at;
