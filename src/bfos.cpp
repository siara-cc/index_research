...
int16_t bfos_node_handler::insertCurrent() {
    ...
    case INSERT_AFTER:
        ...
        *origPos++ &= xFB;
#if BFOS_UNIT_SZ_3 == 1
        updatePtrs(triePos, 6);
        insAt(triePos, ((key_char & xF8) | x05), 5, 0, mask, 0, 0);
        ret = triePos - trie + 4;
#else
        updatePtrs(triePos, 5);
        insAt(triePos, ((key_char & xF8) | x05), 4, mask, 0, 0);
        ret = triePos - trie + 3;
#endif
        *origPos = triePos - origPos;
        break;
    case INSERT_BEFORE:
        key_char = key[keyPos - 1];
        mask = x01 << (key_char & x07);
#if BFOS_UNIT_SZ_3 == 1
        updatePtrs(origPos, 6);
        if (keyPos > 1 && last_child_pos)
            trie[last_child_pos] -= 6;
        if (last_leaf_pos)
            trie[last_leaf_pos] -= 6;
        insAt(origPos, ((key_char & xF8) | x01), 5, 0, mask, 0, 0);
        ret = origPos - trie + 4;
#else
        ...
        if (last_leaf_pos)
            trie[last_leaf_pos] -= 5;
        insAt(origPos, ((key_char & xF8) | x01), 4, mask, 0, 0);
        ret = origPos - trie + 3;
#endif
        break;
    case INSERT_LEAF:
        ...
        *leafPos |= mask;
#else
        leafPos = origPos + ((*origPos & x02) ? 3 : 2);
        if (*origPos & x01) {
            *leafPos |= mask;
        } else {
    ...
    *node_paths = buf;
    do {
        ...
        case 0:
            ...
            continue;
        case 1:
            byte r_children, r_leaves, r_mask;
            t++;
#if BFOS_UNIT_SZ_3 == 1
            r_children = *t++;
            r_leaves = *t++;
...
int16_t bfos_node_handler::locateKeyInLeaf() {
    ...
    byte key_char = *key;
    byte *t = trie;
    last_child_pos = last_leaf_pos = 0;
    do {
        byte trie_char;
        origPos = t;
        trie_char = *t++;
        ...
        case 0:
            last_child_pos = 0;
            last_leaf_pos = t - trie;
            t += *t;
            if (trie_char & x04) {
                byte r_children, r_leaves;
                t = origPos + 2;
#if BFOS_UNIT_SZ_3 == 1
                r_children = *t++;
                r_leaves = *t++;
...
byte bfos_node_handler::ryte_mask[18] = { 0x00, 0x01, 0x03, 0x07, 0x0F, 0x1F,
        0x3F, 0x7F, 0xFF, 0x00, 0x01, 0x03, 0x07, 0x0F, 0x1F,
        0x3F, 0x7F, 0xFF };
byte bfos_node_handler::ryte_incl_mask[8] = { 0x01, 0x03, 0x07, 0x0F, 0x1F,
        0x3F, 0x7F, 0xFF };
byte bfos_node_handler::ryte_leaf_mask[18] = { 0x00, 0x01, 0x03, 0x07, 0x0F, 0x1F,
        0x3F, 0x7F, 0xFF, 0x01, 0x03, 0x07, 0x0F, 0x1F,
        0x3F, 0x7F, 0xFF, 0xFF };
