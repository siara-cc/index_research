    ...
    do {
        ...
        if ((kc ^ tc) < 0x08) {
            ...
            for (int bit = 0; bit < 8; bit++) {
                ...
                if (r_leaves & r_mask) {
                    if (DFOS_NODE_SIZE == 512) {
                        last_ptr = trie[ptr_pos];
                        if (ptr_high_bits & r_mask)
                            last_ptr += 256;
                        ptr_pos++;
                    } else {
                        last_ptr = util::getInt(trie + ptr_pos);
                        ptr_pos += 2;
                    }
                }
                if (children & r_mask)
