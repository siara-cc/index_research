...
char *dfos::get(const char *key, int16_t key_len, int16_t *pValueLen) {
    ...
    if (node.traverseToLeafForGet() < 0)
        return null;
    char *ret = node.getValueAt(pValueLen);
    return ret;
}

int16_t dfos_node_handler::traverseToLeafForGet() {
    while (!isLeaf()) {
        int16_t idx = locateForGet();
        if (idx < 0) {
            idx++;
            idx = ~idx;
        }
...
int16_t dfos_node_handler::locateForGet() {
    byte key_char = *key;
    byte *t = trie;
    byte trie_char = *t++;
    keyPos = 1;
...
int16_t dfos_node_handler::traverseToLeafForPut(byte *node_paths[]) {
    byte level = 1;
    if (node_paths)
        *node_paths = buf;
    while (!isLeaf()) {
        int16_t idx = locateForPut();
        if (idx < 0) {
            idx++;
            idx = ~idx;
        }
...
int16_t dfos_node_handler::locateForPut() {
    byte key_char = *key;
    byte *t = trie;
    byte trie_char = *t;
    keyPos = 1;
    ...
    if (pos < 0) {
        ...
        if (node->isFull(node->key_len + node->value_len)) {
            ...
            if (cmp <= 0) {
                ...
                pos = ~new_block.locateForPut();
                new_block.addData();
            } else {
                node->initVars();
                pos = ~node->locateForPut();
                node->addData();
            }
...
int16_t dfos_node_handler::traverseToLeaf(byte *node_paths[], bool isPut) {
    return 0;
}
int16_t dfos_node_handler::locate(bool isPut) {
    return 0;
}
