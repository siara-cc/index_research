...
public:
    ...
    rb_tree(uint32_t leaf_block_sz = DEFAULT_LEAF_BLOCK_SIZE,
       ...
       bplus_tree_handler<rb_tree>(leaf_block_sz, parent_block_sz, cache_sz, fname, 0, false, opts) {
    ...
    rb_tree(const char *filename, int blk_size, int page_resv_bytes, const uint8_t opts) :
       bplus_tree_handler<rb_tree>(filename, blk_size, page_resv_bytes, opts) {
        init_stats();
    }
